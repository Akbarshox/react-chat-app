...
function TemporaryDrawer(props) {
  ...
  const handleBack = () => {
    if(params.user==='coordinator')
    setRedirect(`/${props.match.params.lang}/${params.user}/appsfeed`);
    else if(params.user==='volunteer')
    setRedirect(`/${props.match.params.lang}/${params.user}/groupfeed`);
    else
    setRedirect(`/${props.match.params.lang}/${params.user}/myapps`);
                ...
                jsonSide.map((n, i) => {
                  return (
                    <Grid key={i} item xs={12} className={style.myProfile} >
                      {n.scroll===''?
                       n.link === 'newReq' || n.link === 'sponsorFeed' ?
                       <Button clicked={(e)=>{props.toggleDrawer('right', false);setTimeout(function(){props.handleOpen()},200)}}
                               btnType={"sideBarButton"}>
                          {n.name[lang]}
                       </Button>:
                      <Link to={`/${lang}/${n.link}`}>
